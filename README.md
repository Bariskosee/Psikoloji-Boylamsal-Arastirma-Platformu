...
> **Project status:** Phases 0–12 are implemented. The full research loop runs: a researcher defines a study, a questionnaire and a longitudinal protocol; a participant joins, installs it to their Home Screen, and is notified as each session opens, with reminders that stop the moment they finish; the researcher monitors compliance per step and per participant, inspects individual responses, and **exports analysis-ready data** in long and wide format with an auto-generated codebook. Missingness is never ambiguous and never a zero: every value column is paired with a status column, and a value appears only where the status says `ANSWERED`. Phase 12's hardening is largely done — dependency audit to zero high or critical findings, operational alerting with runbooks, an **executed** restore drill, automated accessibility checks, password reset by email, and a 500-participant load test. What it could not settle in this environment needs real devices, real data volume and a deployed instance, and is listed honestly in `PLAN.md`: a screen-reader pass, point-in-time restore against the provider, and index behaviour at the size of a finished study. The platform is deployed with Docker Compose on a single always-free VM (ADR-012) rather than on managed hosting: the reconciliation sweepers must run continuously, and no free managed tier provides an always-on process. `docs/runbooks/self-hosted-deploy.md` is the procedure. Next is pilot validation (Phase 13). See `PLAN.md` for the phased roadmap and `CONTRIBUTING.md` to run it locally.
