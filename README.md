...
> **Project status:** Phases 0–10 are implemented. A researcher can define a study, a questionnaire, and a longitudinal protocol; a participant can join it, install it to their Home Screen, and the scheduling engine materialises every session their protocol implies, opens each one at its computed instant, and notifies them — with reminders that stop the moment they finish. The researcher can now **see all of it**: study overview, daily compliance, a participant list with per-step adherence, an individual timeline, a response inspector that renders all seven missingness statuses and never a zero, and an operations page for sweeper liveness. Every compliance figure carries its denominator, and "not yet applicable" is never rendered as 0%. What remains is descriptive analytics and export (Phase 11). See `PLAN.md` for the phased roadmap and `CONTRIBUTING.md` to run it locally.
