...
> **Project status:** Phases 0–11 are implemented. The full research loop runs: a researcher defines a study, a questionnaire and a longitudinal protocol; a participant joins, installs it to their Home Screen, and is notified as each session opens, with reminders that stop the moment they finish; the researcher monitors compliance per step and per participant, inspects individual responses, and **exports analysis-ready data** in long and wide format with an auto-generated codebook. Missingness is never ambiguous and never a zero: every value column is paired with a status column, and a value appears only where the status says `ANSWERED`. What remains is hardening (Phase 12) and pilot validation (Phase 13). See `PLAN.md` for the phased roadmap and `CONTRIBUTING.md` to run it locally.
